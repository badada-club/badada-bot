...
3. Если приложение еще не было создано, то выполняем команду `heroku create [имя приложения]`. Имя приложения для разработки можно выбрать любое.
4. Выставить переменные окружения TELEGRAM_API_TOKEN, TELEGRAM_BOT_USERNAME и APP_NAME командами вроде `heroku config:set TELEGRAM_API_TOKEN=<token>`.
