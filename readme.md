...
5. Cоздать для разработки отдельное (от "продакшна") Heroku-приложение (по очевидным причинам) и не в основном аккаунте (для экономии бесплатных часов аптайма). Название приложения прописать в константу HEROKU_APP_NAME в файле .env, например `HEROKU_APP_NAME=badada-dev-bot`
